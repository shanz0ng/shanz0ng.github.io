...
在函数式编程里，函数跟数据一样——可以赋给变量，可以当参数传给别的方法，也可以首尾串成一条流水线。
...
CC 提供了 F1、F2、F3 三块积木——当你把每一步包成 `Transformer`，`ChainedTransformer` 替你管数据流，就不需要手写中间变量了。
## 四、设计者没预料到的用法

F1、F2、F3 是通用积木。设计者没有限制你能传什么参数——函数式编程本来不该进行限制。
...
`TransformedMap` 恰好提供了这个入口，目的是保证所有写入操作都经过转换。
